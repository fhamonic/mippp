...
GTEST_TEST(grb_model, ctor) { mip_model<grb_traits> model; }

GTEST_TEST(grb_model, add_var) {
    mip_model<grb_traits> model;
...
GTEST_TEST(grb_model, add_var_default_options) {
    ...
    ASSERT_EQ(model.upper_bound(x), mip_model<grb_traits>::infinity);
    ASSERT_EQ(model.type(x), mip_model<grb_traits>::var_category::continuous);
}
...
GTEST_TEST(grb_model, add_var_custom_options) {
    ...
    auto x =
        model.add_var({.obj_coef = 6.14,
                       .lower_bound = 3.2,
                       .upper_bound = 9,
                       .type = mip_model<grb_traits>::var_category::binary});
    ASSERT_EQ(x.id(), 0);
    ASSERT_EQ(model.nb_variables(), 1);
    ASSERT_EQ(model.obj_coef(x), 6.14);
    ASSERT_EQ(model.lower_bound(x), 3.2);
    ASSERT_EQ(model.upper_bound(x), 9);
    ASSERT_EQ(model.type(x), mip_model<grb_traits>::var_category::binary);
}
