...
namespace {
std::size_t total(const check_statistics & s) { return s.feasible + s.infeasible + s.unknown; }
...
TEST(DeletionFilter, StatisticsCountChecksAndNecessity) {
    auto oracle = [](std::span<const std::size_t> ids) {
        ...
        return has(0u) && has(1u) ? feasibility::infeasible : feasibility::feasible;
    };
    for(auto batch : {1u, 2u, 4u}) {
        const auto a = deletion_filter(4, oracle, {.initial_batch_size = batch});
        EXPECT_TRUE(a.irreducible);
        EXPECT_EQ(a.statistics.necessary_members, 2u);
        EXPECT_EQ(a.statistics.unresolved_members, 0u);
        EXPECT_EQ(a.statistics.removed_by_batches + a.statistics.removed_by_singletons, 2u);
        EXPECT_EQ(total(a.statistics.outcomes), a.solve_count);
        EXPECT_EQ(a.statistics.initial_checks + a.statistics.batch_checks + a.statistics.singleton_checks, a.solve_count);
    }
...
TEST(DeletionFilter, DiagnosticSettingsAreSnapshots) {
    ...
    EXPECT_EQ(calls, 0u);
    EXPECT_NE(text.find("No verified conflict is available"), std::string::npos);
    EXPECT_NE(text.find("limits.max_solves: current=0; available="), std::string::npos);
    EXPECT_EQ(a.statistics.models_built(), 0u);
    const auto timed = compute_linear_iis(conflict(), factory, {.limits = {.time_limit = std::chrono::seconds(0)}});
    EXPECT_NE(describe_result(timed).find("limits.time_limit (seconds): current=0; available="), std::string::npos);
    std::stop_source stop;
    stop.request_stop();
    const auto cancelled = compute_linear_iis(conflict(), factory, {.limits = {.stop = stop.get_token()}});
    EXPECT_NE(describe_result(cancelled).find("limits.stop: current=requested; available="), std::string::npos);
    EXPECT_EQ(calls, 0u);
}

TEST(DeletionFilter, ValidationNamesValuesAndOriginalIndices) {
    auto factory = [] { ADD_FAILURE(); return mippp::clp_lp{}; };
    auto s = conflict();
    ...
    s = conflict(); s.rows[1].terms[0].first = 99;
    ...
    s = conflict(); s.rows[1].terms[0].second = std::numeric_limits<double>::quiet_NaN();
    text = invalid_message([&] { (void)compute_linear_iis(s, factory); });
    EXPECT_NE(text.find("coefficient: current=nan; available="), std::string::npos);
    s = conflict(); s.variables[0].integer = true;
    text = invalid_message([&] { (void)compute_linear_iis(s, factory); });
    EXPECT_NE(text.find("policy.analyzed_domain: current=original; available=original, lp_relaxation"), std::string::npos);
    EXPECT_NE(text.find("cannot explain"), std::string::npos);
    constexpr linear_policy native{.native_seed = true};
    text = invalid_message([&] { (void)compute_linear_iis<native>(conflict(), factory,
        {.native = {.relative_tolerance = -0.25}}); });
    EXPECT_NE(text.find("native.relative_tolerance: current=-0.25; available="), std::string::npos);
}
...
TEST(DeletionFilter, PortableSolverIssuesRetainObservedFacts) {
    ...
    EXPECT_EQ(s.last_solver_issue, solve_issue::time_limit); // history is not erased
    detail::record_solver_issue(std::variant<mippp::status::optimal_infeasible_unscaled>{}, s);
    EXPECT_EQ(s.last_solver_issue, solve_issue::numerical);
    detail::record_solver_issue(std::variant<mippp::status::primal_and_dual_infeasible>{}, s);
    EXPECT_EQ(s.last_solver_issue, solve_issue::ambiguous);
    detail::record_solver_issue(std::variant<mippp::status::limit_reached>{}, s);
...
TEST(DeletionFilter, LicenseFailuresRemainCatchableAndExplainExpansion) {
    constexpr linear_policy policy{.elasticity = elasticity_strategy::reuse, .deletion = deletion_strategy::reuse};
    try {
        ...
        EXPECT_NE(text.find("policy.deletion: current=reuse; available=rebuild, reuse"), std::string::npos);
        EXPECT_NE(text.find("policy.elasticity: current=reuse; available=off, rebuild, reuse"), std::string::npos);
        EXPECT_NE(text.find("2 variables, 2 rows, 6 separate"), std::string::npos);
        EXPECT_NE(text.find("Solver details: test license refused"), std::string::npos);
        try { std::rethrow_if_nested(e); FAIL(); }
        catch(const mippp::license_error & original) { EXPECT_STREQ(original.what(), "test license refused"); }
    }
    try {
        (void)compute_linear_iis(conflict(), []() -> mippp::clp_lp { throw std::runtime_error("caller failure"); });
        FAIL();
    } catch(const std::runtime_error & e) { EXPECT_STREQ(e.what(), "caller failure"); }
}
...
TEST(ClpRay, DiagnosticAccountingAcrossStrategiesAndLimits) {
    auto run = [&]<linear_policy Policy>() {
        for(std::size_t limit = 0; limit < 12; ++limit) {
            unsigned builds = 0;
            const auto a = compute_linear_iis<Policy>(conflict(), [&] { ++builds; return mippp::clp_lp{}; },
            ...
            EXPECT_EQ(total(s.feasibility_checks) + total(s.elastic_checks), a.reduction.solve_count);
            EXPECT_EQ(s.solver_runs() + s.rebuild.bound_only_proofs + s.rebuild.solves_skipped +
                s.deletion.solves_skipped + s.elastic.solves_skipped, a.reduction.solve_count);
    ...
    run.template operator()<linear_policy{.deletion = deletion_strategy::reuse}>();
    run.template operator()<linear_policy{.elasticity = elasticity_strategy::reuse,
        .deletion = deletion_strategy::reuse, .native_seed = true, .prune_bounds = true}>();
}

TEST(ClpRay, DiagnosticBoundShortcutsAndCancellation) {
    auto s = conflict(); s.variables[0].lower = 2.;
    auto a = compute_linear_iis(s, [] { return mippp::clp_lp{}; });
    EXPECT_GT(a.statistics.rebuild.bound_only_proofs, 0u);
    EXPECT_EQ(a.statistics.solver_runs() + a.statistics.rebuild.bound_only_proofs, a.reduction.solve_count);
    std::stop_source stop;
    a = compute_linear_iis(conflict(), [&] { stop.request_stop(); return mippp::clp_lp{}; },
...
TEST(ClpRay, DiagnosticSeedOutcomesAndOptionalTiming) {
    constexpr linear_policy policy{.native_seed = true, .prune_bounds = true, .measure_time = true};
    const auto a = compute_linear_iis<policy>(conflict(), [] { return mippp::clp_lp{}; });
    ...
    EXPECT_NE(report.find("Variable 0 upper bound (original zero-based index)"), std::string::npos);
    EXPECT_NE(report.find("Row 0 lower bound (original zero-based index)"), std::string::npos);
    ASSERT_TRUE(a.statistics.elapsed);
    EXPECT_GE(a.statistics.elapsed->count(), 0.);
    const auto rejected = compute_linear_iis<policy>(conflict(), [] { return mippp::clp_lp{}; },
        {.native = {.relative_tolerance = 1.}});
    EXPECT_EQ(rejected.diagnostics.native_seed, seed_outcome::verification_feasible);
    EXPECT_TRUE(rejected.diagnostics.full_set_fallback);
    EXPECT_NE(describe_result(rejected).find("native.relative_tolerance: current=1; available="), std::string::npos);
    const auto stopped = compute_linear_iis<policy>(conflict(), [] { return mippp::clp_lp{}; }, {.limits = {.max_solves = 1}});
    EXPECT_EQ(stopped.diagnostics.native_seed, seed_outcome::verification_skipped);
...
TEST(ClpRay, DiagnosticElasticNoProgress) {
    ...
    EXPECT_NE(describe_result(a).find("elastic.violation_tolerance: current=1000000; available="), std::string::npos);
}

TEST(DeletionFilter, DiagnosticSummariesRespectProofState) {
    linear_result a;
    EXPECT_NE(describe_result(a).find("No verified conflict"), std::string::npos);
    ...
    EXPECT_NE(describe_result(a).find("does not establish feasibility of the original integer problem"), std::string::npos);
    a.reduction.initial_status = feasibility::infeasible;
    a.reduction.irreducible = true;
    EXPECT_NE(describe_result(a).find("fixed requirements conflict"), std::string::npos);
}
