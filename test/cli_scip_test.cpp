...
using M = mip_model<cli_scip_traits>;
...
GTEST_TEST(cli_scip_model, get_objective) { model_tests<M>::get_objective(); }

GTEST_TEST(cli_scip_model, add_and_get_constraint) {
    model_tests<M>::add_and_get_constraint();
}
GTEST_TEST(cli_scip_model, build_optimize) { model_tests<M>::build_optimize(); }
