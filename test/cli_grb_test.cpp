...
GTEST_TEST(cli_grb_model, ctor) { mip_model<cli_cbc_traits> model; }

GTEST_TEST(cli_grb_model, add_var) {
...
GTEST_TEST(cli_grb_model, add_var_default_options) {
    mip_model<cli_cbc_traits> model;
    auto x = model.add_var();
    ASSERT_EQ(x.id(), 0);
    ASSERT_EQ(model.nb_variables(), 1);
...
GTEST_TEST(cli_grb_model, add_var_custom_options) {
    mip_model<cli_cbc_traits> model;
    auto x =
    ...
    ASSERT_EQ(model.type(x), mip_model<cli_cbc_traits>::var_category::binary);
}

GTEST_TEST(cli_grb_model, add_vars) {
    mip_model<cli_cbc_traits> model;
...
GTEST_TEST(cli_grb_model, add_get_constraint) {
    mip_model<cli_cbc_traits> model;
    auto x = model.add_var();
    auto y = model.add_var();
    auto z = model.add_var();
...
GTEST_TEST(cli_grb_model, build_optimize) {
    ...
    ASSERT_EQ(solver_model.optimize(),
              mip_model<cli_cbc_traits>::ret_code::success);
