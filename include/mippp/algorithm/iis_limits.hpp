...
enum class termination { completed, indeterminate, solve_limit, time_limit, cancelled };
...
struct options {
    ...
    std::chrono::duration<double> time_limit{std::numeric_limits<double>::infinity()};
};

namespace detail {
using budget_clock = std::chrono::steady_clock;

inline options normalize_limits(options opts,
                                budget_clock::time_point now = budget_clock::now()) {
    const auto seconds = opts.time_limit.count();
    if(std::isnan(seconds) || seconds < 0)
        throw std::invalid_argument("The conflict search time limit is invalid. " +
            setting_help("limits.time_limit (seconds)", setting_value(seconds),
                         "0 to stop immediately, a positive duration, or infinity for no relative limit"));
    ...
    if(opts.time_limit < std::chrono::duration<double>(available))
        opts.deadline = std::min(opts.deadline, now +
            std::chrono::duration_cast<budget_clock::duration>(opts.time_limit));
...
public:
    ...
    void consume(std::size_t calls) {
        if(calls > limits_.max_solves - used_)
            throw std::logic_error("Internal conflict-search accounting error: reported " + setting_value(calls) +
                " checks with only " + setting_value(limits_.max_solves - used_) +
                " remaining. Please report this as a library bug; increasing the limit is not a fix.");
        used_ += calls;
    }
};
}  // namespace detail
