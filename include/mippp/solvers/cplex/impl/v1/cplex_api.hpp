...
public:
    ...
    static constexpr const char * key = "CPLEX";
    static constexpr std::array library_names = {"cplex2220", "cplex2212",
                                                 "cplex2211", "cplex2210"};
...
private:
    explicit cplex_api(detail::dynamic_library && library)
        ...
        // VVRRMMFF, 22010200 for 22.1.2.0: the fix varies within one
        // libcplexVVRRMM.so, so it is not part of the claim
        int number;
        _check(env, versionnumber(env, &number));
        check_library_version(
            {number / 1000000, number / 10000 % 100, number / 100 % 100});
        _close_env(env);
    }
...
public:
    ...
    void _check(CPXENVptr env, const int retcode) const {
        ...
        if(retcode == 1016) throw license_error(detail::license_diagnostic("CPLEX", errmsg).c_str());
