...
MSKrescodee MSK_getversion(MSKint32t * major, MSKint32t * minor,
                           MSKint32t * revision);
...
public:
    ...
    static constexpr const char * key = "MOSEK";
    ...
    static constexpr std::array validated_versions = {
        solver_version_range{{11, 0}, {11, 3}}};
...
private:
    explicit mosek_api(detail::dynamic_library && library)
        ...
        _check(getversion(&major, &minor, &revision));
        check_library_version({major, minor, revision});
    }
