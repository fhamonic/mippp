...
public:
    ...
    static constexpr const char * key = "CBC";
    // Ubuntu ships a libCbc.so without the C API next to libCbcSolver.so,
    // hence the probe
    static constexpr std::array library_names = {"CbcSolver", "Cbc"};
...
private:
    explicit cbc_api(detail::dynamic_library && library)
        ...
        check_library_version(getVersion());
    }
};
