...
public:
    ...
    static constexpr const char * key = "COPT";
...
private:
    explicit copt_api(detail::dynamic_library && library)
        ...
        // "Cardinal Optimizer v8.0.5. Build date ...": the only version
        // the C API reports is the number after the v
        char banner[COPT_BUFFSIZE];
        GetBanner(banner, COPT_BUFFSIZE);
        const std::string_view text(banner);
        const std::size_t digit = text.find_first_of("0123456789");
        check_library_version(
            digit == std::string_view::npos ? text : text.substr(digit));
    }
