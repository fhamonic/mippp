...
// model paths. They must differ only in how solver state survives between
// calls.
template <typename Model, typename Scalar>
auto build_elastic_lp(
    Model & model, std::size_t num_variables,
    ...
    for(std::size_t i = 0; i < num_variables; ++i)
        variables.push_back(model.add_variable({.obj_coef = Scalar{0},
    ...
    for(std::size_t id = 0; id < inequalities.size(); ++id) {
        ...
        if(!fixed[id]) {
            auto slack = model.add_variable({.obj_coef = Scalar{1},
...
elastic_trial read_elastic_trial(
    ...
    Scalar tolerance, work_statistics & stats,
    ...
    const bool optimal = std::visit(
        [](const auto & value) {
            return std::derived_from<std::decay_t<decltype(value)>,
                                     status::optimal>;
        },
        status_value);
    if(!optimal) return {};
...
elastic_trial solve_elastic_lp(
    ...
    stats.model_built(std::max(num_variables + slacks.size(), std::size_t{1}),
                      inequalities.size());
...
public:
    ...
    elastic_lp_workspace(
        std::size_t num_variables,
        const std::vector<linear_inequality<Scalar>> & inequalities,
        Factory & factory, Scalar tolerance, options limits,
        ...
        , stats_(stats) {
        stats_.model_built(
            std::max(num_variables + slacks_.size(), std::size_t{1}),
            inequalities.size());
    }
    ...
    elastic_trial operator()(std::span<const std::size_t> hard) {
        ...
        // dimensions intact. No infinity conversion or basis remapping is
        // needed. Construction uses an empty hard set, so every ID has a slack
        // at the same index; unlike the cold path, no slack columns are
        // omitted.
        for(auto id : hard) {
            if(!fixed_[id]) {
