...
template <linear_policy Policy = {}, typename Factory,
...
[[nodiscard]] linear_result compute_linear_iis(
    ...
    if constexpr(Policy.native_seed) {
        if(!std::isfinite(config.native.relative_tolerance) ||
           config.native.relative_tolerance < 0)
            throw std::invalid_argument(
                "The solver-hint threshold is invalid. " +
                detail::setting_help(
                    "native.relative_tolerance",
                    detail::setting_value(config.native.relative_tolerance),
                    "any finite number >= 0 (default 1e-9)") +
                " Smaller values retain more evidence; values >= 1 can discard "
                "the entire hint.");
    }
    if constexpr(use_elastic) {
        if(!std::isfinite(config.elastic.violation_tolerance) ||
           config.elastic.violation_tolerance < 0)
            throw std::invalid_argument(
                "The soft-constraint threshold is invalid. " +
                detail::setting_help(
                    "elastic.violation_tolerance",
                    detail::setting_value(config.elastic.violation_tolerance),
                    "any finite number >= 0 (default 1e-7)"));
    }
    const detail::prepared_linear_system<Scalar> prepared(system, mode,
                                                          milp_model<Model>);
    ...
    if constexpr(Policy.native_seed)
        diagnostics.native_seed =
            original_mip ? seed_outcome::integer_model
                         : (native_capable ? seed_outcome::not_reached
                                           : seed_outcome::unsupported);
    if constexpr(use_elastic)
        diagnostics.elastic_seed = original_mip ? seed_outcome::integer_model
                                                : seed_outcome::not_reached;
    ...
    std::conditional_t<native_capable,
                       detail::native_seed_data<Policy.order_by_weight>,
                       detail::disabled_feature>
        native_data;
    std::conditional_t<reuse_deletion,
                       std::optional<detail::deletion_workspace<Model, Scalar>>,
                       detail::disabled_feature>
        deletion_model;
    auto rebuilt_model = [&] {
    ...
    auto check_model = [&](std::span<const std::size_t> active) {
            ...
            if(!capture_certificate) {
                if(!deletion_model) {
                    ...
                    for(const auto & v : system.variables)
                        integer_variables.push_back(v.integer &&
                                                    mode == domain::original);
                    deletion_model.emplace(integer_variables,
                                           prepared.inequalities(), factory,
                                           opts, statistics.deletion);
                }
        ...
        if constexpr(needs_rebuilt_model) {
            return rebuilt_model(active, [&](auto & model, auto ids) {
                if constexpr(native_capable) {
                    if(capture_certificate) {
                        ...
                        native_data =
                            detail::collect_native_seed<Policy.prune_bounds,
                                                        Policy.order_by_weight>(
            ...
            // No native provider can set capture_certificate in this
            // specialization. The retained path above always returns.
            throw std::logic_error(
                "Internal conflict-search error: a disabled model-building "
                "path was requested. "
                "Please report this as a library bug; changing solver settings "
                "is not a fix.");
        }
    };
    ...
    auto has_weights = [&] {
        if constexpr(weighted)
            return !native_data.weights.empty();
        else
            return false;
    };
    auto full_order = [&](std::size_t a, std::size_t b) {
        if constexpr(!std::same_as<Order, input_order>) {
            if(std::invoke(config.order, candidates[a], candidates[b]))
                return true;
            if(std::invoke(config.order, candidates[b], candidates[a]))
                return false;
        }
        if constexpr(weighted) {
            if(has_weights() &&
               native_data.weights[a] != native_data.weights[b])
        ...
        if constexpr(!use_elastic && !native_capable) {
            ...
            auto try_seed =
                ...
                auto filtered = [&] {
                    if constexpr(std::same_as<Order, input_order>) {
                        if(!has_weights())
                            return deletion_filter(seed.size(), seeded_oracle,
                                                   budget.remaining());
                    }
                    return deletion_filter(
                        seed.size(), seeded_oracle, budget.remaining(),
                        [&](auto a, auto b) {
                            return full_order(seed[a], seed[b]);
                        });
                }();
                ...
                outcome =
                    first_check
                        ...
                        : (filtered.initial_status == feasibility::feasible
                               ? seed_outcome::verification_feasible
                           : filtered.initial_status == feasibility::unknown
                               ? seed_outcome::verification_unknown
                               : seed_outcome::used);
            ...
            if constexpr(native_capable) {
                if(native_data.members &&
                   native_data.members->size() < candidates.size()) {
                    if(auto filtered = try_seed(
                           *native_data.members, diagnostics.native_seed,
            ...
            if constexpr(use_elastic) {
                ...
                const auto filter_opts =
                    budget.remaining(config.elastic.max_solves);
                auto seed = [&] {
                    if constexpr(Policy.elasticity ==
                                     elasticity_strategy::reuse &&
                                 has_modifiable_variable_bounds<Model>) {
                        // Lazy construction: a stopped filter never loads a
                        // solver.
                        std::optional<
                            detail::elastic_lp_workspace<Model, Scalar>>
                            workspace;
                        auto retained_oracle =
                            [&](std::span<const std::size_t> hard) {
                                if(!workspace)
                                    workspace.emplace(
                                        system.variables.size(), inequalities,
                                        factory,
                                        static_cast<Scalar>(
                                            config.elastic.violation_tolerance),
                                        opts, statistics.elastic);
                                return (*workspace)(hard);
                            };
                        auto answer = elasticity_filter(
                            candidates.size(), retained_oracle, filter_opts);
                        if(workspace)
                            elasticity_reoptimizations =
                                workspace->reoptimizations();
                        return answer;
                    } else {
                        auto elastic_oracle =
                            [&](std::span<const std::size_t> hard) {
                                return detail::solve_elastic_lp(
                                    system.variables.size(), inequalities, hard,
                                    factory,
                                    static_cast<Scalar>(
                                        config.elastic.violation_tolerance),
                                    opts, statistics.elastic);
                            };
                        return elasticity_filter(candidates.size(),
                                                 elastic_oracle, filter_opts);
                    }
                }();
                elasticity_calls = seed.solve_count;
                statistics.elastic_checks = seed.outcomes;
                statistics.elastic_seed_size = seed.members.size();
                diagnostics.elastic_seed =
                    seed.no_progress
                        ? seed_outcome::no_progress
                        : (seed.reason == termination::indeterminate
                               ? seed_outcome::inconclusive
                               : seed_outcome::limit_reached);
                budget.consume(seed.solve_count);
                if(seed.proven_infeasible) {
                    if(auto filtered =
                           try_seed(seed.members, diagnostics.elastic_seed,
            ...
            // The full set is still known to conflict. Restore its active
            // bounds in a retained model and continue without checking it
            // again.
            known.solve_count = budget.used();
            diagnostics.full_set_fallback = true;
            if constexpr(std::same_as<Order, input_order>) {
                if(!has_weights())
                    return detail::deletion_filter_impl(
                        std::move(known), oracle, opts, input_order{});
            }
            return detail::deletion_filter_impl(std::move(known), oracle, opts,
                                                full_order);
        }
    };
    auto reduced = [&] {
        try {
            ...
            const auto message =
                detail::solver_failure_context(true, diagnostics, statistics) +
                "\nSolver details: " + error.what();
            std::throw_with_nested(license_error(message.c_str()));
        } catch(const solver_error & error) {
            const auto message =
                detail::solver_failure_context(false, diagnostics, statistics) +
                "\nSolver details: " + error.what();
    ...
    const auto native_seed_size = [&]() -> std::size_t {
        if constexpr(native_capable)
            return native_data.members ? native_data.members->size() : 0;
        else
            return 0;
    }();
    const auto deletion_model_reused = [&] {
        if constexpr(reuse_deletion)
            return deletion_model.has_value();
        else
            return false;
    }();
    const auto deletion_reoptimizations = [&]() -> std::size_t {
        if constexpr(reuse_deletion)
            return deletion_model ? deletion_model->reoptimizations() : 0;
        else
            return 0;
    }();
    ...
    if constexpr(reuse_deletion)
        deletion_model
            .reset();  // include solver cleanup in optional elapsed time
    ...
    if constexpr(Policy.measure_time)
        answer.statistics.elapsed = detail::budget_clock::now() - started;
    return answer;
}
