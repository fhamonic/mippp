...
struct prepared_linear_system {
    ...
    prepared_linear_system(const linear_system<Scalar> & input, domain analyzed,
        ...
        // Validate and assign stable candidate IDs once, before any solver
        // calls. Each finite side is independent: for example, only the lower
        // side of 2 <= x <= 5 may conflict with a separate bound x <= 1.
        ...
        auto add_bound = [&](std::optional<Scalar> bound, member_kind kind,
            ...
            if(!std::isfinite(*bound))
                throw std::invalid_argument(
                    "Invalid bound at " +
                    std::string(kind == member_kind::row_lower ||
                                        kind == member_kind::row_upper
                                    ? "row "
                                    : "variable ") +
                    std::to_string(index) + " (zero-based index). " +
                    setting_help(
                        kind == member_kind::row_lower ||
        ...
        for(std::size_t i = 0; i < system.variables.size(); ++i) {
            ...
            if(v.integer && mode == domain::original && !supports_integer)
                throw std::invalid_argument(
                    ...
                    " requires whole-number values, but the selected solver "
                    "model does not support them. "
                    "Choose an integer-capable model to explain the original "
                    "problem. " +
                    setting_help("policy.analyzed_domain", "original",
                                 "original, lp_relaxation") +
                    " lp_relaxation allows fractional values and cannot "
                    "explain conflicts caused only by integer requirements.");
        ...
        for(std::size_t i = 0; i < system.rows.size(); ++i) {
            ...
            for(const auto & [column, coefficient] : row.terms) {
                if(column >= system.variables.size())
                    throw std::invalid_argument(
                        "Row " + std::to_string(i) + " refers to variable " +
                        std::to_string(column) +
                        " (zero-based indices), but the problem has " +
                        std::to_string(system.variables.size()) +
                        " variables. " +
                        (system.variables.empty()
                             ? "No variable references are valid; add the "
                               "variable first."
                             : "Use indices 0 through " +
                                   std::to_string(system.variables.size() - 1) +
                                   "."));
                if(!std::isfinite(coefficient))
                    throw std::invalid_argument(
                        "Row " + std::to_string(i) + ", variable " +
                        std::to_string(column) + " (zero-based indices): " +
                        setting_help("coefficient", setting_value(coefficient),
                                     "any finite number"));
            }
            ...
            if(kind == member_kind::row_lower ||
               kind == member_kind::row_upper) {
                ...
                inequalities.push_back(
                    {row.terms, lower ? *row.lower : *row.upper, lower});
            } else {
