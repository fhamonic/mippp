...
enum class seed_outcome {
    disabled, not_reached, unsupported, integer_model, unavailable,
    invalid_certificate, not_smaller, no_progress, inconclusive, limit_reached,
    verification_skipped, verification_feasible, verification_unknown, used
};

enum class solve_issue {
    none, time_limit, iteration_limit, node_limit, solution_limit, memory_limit, other_limit,
    interrupted, numerical, ambiguous, no_solution, failed
};

struct work_statistics {
    std::size_t models_built = 0;
    std::size_t solver_runs = 0; // Model::solve calls, not hidden internal optimizer passes
    ...
    // means not observed, NOT unlimited. No extra native queries for statistics.
...
struct linear_statistics {
    ...
    [[nodiscard]] std::size_t models_built() const noexcept {
        return rebuild.models_built + deletion.models_built + elastic.models_built;
    }
};
