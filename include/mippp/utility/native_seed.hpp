...
struct native_seed_data {
    ...
    [[no_unique_address]] std::conditional_t<OrderByWeight,
        std::vector<long double>, disabled_feature> weights;
};
...
template <bool PruneBounds = false, bool OrderByWeight = false,
          typename Model, std::floating_point Scalar>
native_seed_data<OrderByWeight> collect_native_seed(Model & model, std::span<const std::size_t> active,
    ...
    for(auto id : active) {
        ...
        expected_rows += kind == member_kind::row_lower || kind == member_kind::row_upper;
    }
    ...
    if constexpr(has_infeasibility_certificate<Model>) {
        if(auto certificate = model.get_infeasibility_certificate()) {
            ...
            const auto finite = [](const auto & values) {
                return std::all_of(values.begin(), values.end(), [](auto v) { return std::isfinite(v); });
            };
            if(c.row_lower.size() == rows && c.row_upper.size() == rows &&
               c.variable_lower.size() == columns && c.variable_upper.size() == columns &&
               finite(c.row_lower) && finite(c.row_upper) &&
               finite(c.variable_lower) && finite(c.variable_upper)) {
                ...
                for(auto id : active) {
                    ...
                    switch(kind) {
                        case member_kind::row_lower: weights.push_back(c.row_lower[row++]); break;
                        case member_kind::row_upper: weights.push_back(c.row_upper[row++]); break;
                        case member_kind::variable_lower: weights.push_back(c.variable_lower[index]); break;
                        case member_kind::variable_upper: weights.push_back(c.variable_upper[index]); break;
                    }
                }
                if(auto support = ray_support<Scalar>(weights, native.relative_tolerance)) {
                    if constexpr(OrderByWeight) {
                        certificate_weights.assign(candidates.size(), 0);
                        for(std::size_t i = 0; i < active.size(); ++i)
                            certificate_weights[active[i]] = std::abs(static_cast<long double>(weights[i]));
                    }
                    native_seed.emplace();
        ...
        if(auto ray = model.get_infeasibility_ray()) {
            ...
            for(auto id : active) {
                ...
                if(kind == member_kind::row_lower || kind == member_kind::row_upper)
                    rows.push_back(id);
                else bounds.push_back(id);
            }
            if(ray->size() == rows.size()) {
                if(auto support = ray_support<Scalar>(*ray, native.relative_tolerance)) {
                    if constexpr(OrderByWeight || PruneBounds) {
                        auto columns = ray_column_magnitudes<Scalar>(system.variables.size(), *ray,
                            [&](std::size_t row) -> const auto & {
                                return system.rows[candidates[rows[row]].index].terms;
                            });
                        if constexpr(OrderByWeight) {
                            long double scale = 0;
                            for(auto value : *ray)
                            ...
                            certificate_weights.assign(candidates.size(),
                                std::numeric_limits<long double>::max());
                            for(std::size_t i = 0; i < rows.size(); ++i)
                                certificate_weights[rows[i]] = std::abs(static_cast<long double>((*ray)[i])) / scale;
                            if(columns) for(auto id : bounds)
                                certificate_weights[id] = (*columns)[candidates[id].index];
                        }
                        if constexpr(PruneBounds) {
                            if(columns) {
                                const auto largest = columns->empty() ? 0.L
                                    : *std::max_element(columns->begin(), columns->end());
                                std::erase_if(bounds, [&](auto id) {
                                    return largest == 0 ||
                                        (*columns)[candidates[id].index] / largest <= native.relative_tolerance;
                                });
                            }
                        }
                    }
    ...
    if(native_seed) answer.outcome = native_seed->size() < candidates.size()
        ? seed_outcome::verification_skipped : seed_outcome::not_smaller;
    return answer;
}
} // namespace mippp::iis::detail
