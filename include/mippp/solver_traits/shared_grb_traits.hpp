...
struct shared_grb_traits {
    ...
    struct solver_wrapper : public abstract_solver_wrapper {
        ...
        [[nodiscard]] solver_wrapper(const auto & m_model)
            ...
            double * col_lb =
                const_cast<double *>(m_model.column_lower_bounds());
            double * col_ub =
                const_cast<double *>(m_model.column_upper_bounds());
            typename traits::var_category * vtype =
                const_cast<typename traits::var_category *>(
                    m_model.column_types());
            ...
            if(error)
                std::cerr << "GRBstartenv" << error << ": "
                          << GRBgeterrormsg(this->env) << std::endl;
            error =
                GRBnewmodel(this->env, &this->model, "PL", nb_vars, obj, col_lb,
                            col_ub, reinterpret_cast<char *>(vtype), NULL);
            if(error)
                std::cerr << "GRBnewmodel" << error << ": "
                          << GRBgeterrormsg(this->env) << std::endl;
            error =
                GRBaddrangeconstrs(this->model, nb_rows, nb_elems, row_begins,
        ...
        [[nodiscard]] solver_wrapper(GRBenv * e, GRBmodel * m)
            : env(e), model(m) {}
        ~solver_wrapper() {
        ...
        [[nodiscard]] double get_objective_value() const noexcept {
            double obj;
            GRBgetdblattr(const_cast<GRBmodel *>(model), GRB_DBL_ATTR_OBJVAL,
                          &obj);
            return obj;
        }
    };
