...
struct shared_scip_traits {
    ...
    struct solver_wrapper : public abstract_solver_wrapper {
        ...
        [[nodiscard]] solver_wrapper(const auto & model) : model(nullptr) {
            using traits =
                typename std::decay_t<decltype(m_model)>::solver_traits;
            SCIPcreate(&model);  // Creating the SCIP environment
            /* include default plugins */
            SCIPincludeDefaultPlugins(model);
            // Creating the SCIP Problem.
            SCIPcreateProbBasic(model, "solver_wrapper");
            ...
            this->vars = std::vector<SCIP_VAR *>(
                static_cast<std::size_t>(nb_vars), nullptr);
            for(int i = 0; i < nb_vars; ++i) {
                ...
                if(SCIPaddVar(this->model, this->vars[i]) != SCIP_OKAY) {
            ...
            for(int i = 0; i < nb_rows; ++i) {
                ...
                if(SCIP_RETCODE code = SCIPcreateConsBasicLinear(
                       this->model, &this->constrs[i], name.c_str(), 0, nullptr,
                       nullptr, row_lb[i], row_ub[i]);
                   code != SCIP_OKAY) {
                    std::cerr << "SCIPcreateConsBasicLinear not okay"
                              << std::endl;
                    SCIPerrorMessage("Error <%d> in function call\n", code);
                }

                for(int elem_num = begin_elems; elem_num != end_elems;
                    ++elem_num) {
                    SCIPaddCoefLinear(this->model, this->constrs[i],
                                      this->vars[indices[elem_num]],
                                      coefs[elem_num]);
                }

                SCIPaddCons(this->model, this->constrs[i]);
            }
        }
        ~solver_wrapper() {
            for(auto & var : vars) {
    ...
    [[nodiscard]] static solver_wrapper build(const auto & model) {
        return solver_wrapper(model);
    }
};
