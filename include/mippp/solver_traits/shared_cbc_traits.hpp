...
struct shared_cbc_traits {
    ...
    struct solver_wrapper : public abstract_solver_wrapper {
        ...
        [[nodiscard]] explicit solver_wrapper(const auto & m_model) {
            using traits =
        ...
        int optimize() noexcept {
            ...
            std::ranges::transform(parameters, argv.begin(),
                                   &std::string::c_str);
            argv[parameters.size()] = "-solve";

            CbcSolverUsefulData solverData;
            CbcMain0(model, solverData);
            CbcMain1(static_cast<int>(parameters.size() + 1), argv.data(),
                     model, &cbc_callback, solverData);
